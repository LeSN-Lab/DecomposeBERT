...
class ConcernIdentificationBert:
    ...
    def propagate(self, module, input_tensor, sparse_mode=False):
    ...
    def propagate_encoder_block(self, ref_model, module, input_tensor):
        def ff1_hook(module, input, output):
            ...
            if not self.sparse_mode and torch.sum(current_weight != 0) > torch.numel(current_weight) * 0.9:
                self.remove(module, output[:, 0, :])
            elif self.sparse_mode and torch.sum(current_weight != 0) > torch.numel(current_weight) * 0.4:
